...
## RL0 rememberance.dat schema

`PolyformTown/data/rl0/rememberance.dat` is generated by
`PolyformTown/bin/rl0_generate` together with `completions.dat`.  It uses
deletion-style indexed item arcs.

Rows are grouped by key length.  Each alternative value is printed on
its own indented line for auditability:
...
An empty value list marks a full/maximal row:
...
An arc is either `[]` or an ordered list of indexed parity items:

```text
[[p,i],[p,i],...]
```

The semantic assertion of each value is:

```text
canonical(key_arc + value_arc) is a valid full vertex figure
```

Full/maximal valid figures are included as rows whose value list is empty.
Missing keys are treated as false/invalid by checkers.
