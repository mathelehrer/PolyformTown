...
%% A breakable tcolorbox forces a page-break opportunity right before
%% itself (it does \@nobreakfalse\addpenalty0 internally), so if a
%% heading lands too low the box jumps to the next page and the heading
%% is stranded at the foot of the current one.  Two measures fix this:
%%
%% 1. \subsectionbreak = \nobreak  — forbids a break between a section
%%    heading and an immediately following subsection (replaces the
%%    default negative \@secpenalty that *rewards* such a break).
%% 2. \needspace before every heading — if there isn't room for the
%%    heading plus the start of its box, the whole unit is moved to the
%%    next page instead of leaving the heading behind.
