...
What it does (all for arrangement #13, the gap-free v[12]-type centre):
  ...
  4. Converts everything back to CSV (x,y,dir,ref,pt):
       from_python/SUPER_SUPER_H8.csv      -- copy 0 (0°, base)
       from_python/SUPER_SUPER_H8_120.csv  -- copy 1 (120°)
       from_python/SUPER_SUPER_H8_240.csv  -- copy 2 (240°)
...
o = open("tmp/rot_overlay_super_super_h8.svg", "w")
...
o.write("</svg>\n"); o.close()
print("wrote tmp/rot_overlay_super_super_h8.svg")
    ...
    with open(fn, "w") as f:
        ...
        for h in hats:
            f.write("%.12g,%.12g,%d,%s,%d\n"
                    % (h["x"]-C13[0], h["y"]-C13[1], h["dir"], "True" if h["ref"] else "False", h["pt"]))
# super cluster: one file per copy (0° / 120° / 240°)
for ci, (cp, suffix) in enumerate(zip(copies, ["", "_120", "_240"])):
    fn = f"from_python/SUPER_SUPER_H8{suffix}.csv"
    write_csv(fn, cp)
...
write_csv("tmp/ROT_LEVEL.csv", [super_hats[si] for si in order])
print("wrote tmp/ROT_LEVEL.csv (%d hats; per-level %s)"
