/*
 * grow_symmetric2.c - grow hat tile clusters around a 3-fold symmetric seed
 *                     at vertex 12 of the base hat.
 *
 * The seed is a "pinwheel" of three hats all sharing vertex 12 (0-based),
 * rotated 0°, 120°, and 240°.  v[12] = (3,1,-1), a valence-3 (triangular)
 * lattice vertex with 120° interior angle.  Unlike v[0], it is NOT the
 * coordinate origin, so the rotation-around-centre transform requires a
 * translation of (m6=+1, n6=-1) after the t=2 lattice rotation.
 *
 * Build:  make grow_symmetric2
 * Run:    ./bin/grow_symmetric2 [--level N] [--tile preferences/focus.tile]
 *                               [--remembrance data/rl0/remembrance.dat]
 *                               [--deletions data/rl0/deletions.dat]
 *                               [--out-dir data/sym12] [--svg out.svg]
 */
...
#define CENTER_VERTEX   12   /* v[12] = (3,1,-1) */
...
/*
 * Translation applied after a t=2 rotation to bring the rotation centre
 * (hat.v[12]) back to its original position.  Computed in build_symmetric_seed.
 */
static int g_rot_m6, g_rot_n6;

/*
 * Build the 3-hat pinwheel seed sharing hat.v[12].
 *
 * Attaches rot[1] (120°) and rot[2] (240°) to the growing polygon,
 * accepting only (be,te) pairs where the aligned tile's v[12] coincides
 * with the centre vertex.
 */
                                 ...
                                 Cycle *center_out)
{
    ...
    /* Compute the translation that undoes a t=2 rotation around `center`. */
    Cycle tmp; memset(&tmp, 0, sizeof(tmp));
    tmp.n = 1; tmp.v[0] = center;
    Cycle rtmp;
    cycle_transform_lattice(&tmp, &rtmp, tile->lattice, 2);
    int dx = center.x - rtmp.v[0].x;
    int dy = center.y - rtmp.v[0].y;
    if (!tetrille_delta_to_6(center.v, dx, dy, &g_rot_m6, &g_rot_n6)) {
        fputs("ERROR: center vertex is not at a tetrille-translatable position\n", stderr);
        return 0;
    }

    /* Build rotated variants: t=0 (base), t=2 (120°), t=4 (240°). */
    Cycle rot[SEED_HATS];
    rot[0] = tile->base;
    ...
    for (int k = 1; k < SEED_HATS; k++) {
        ...
        if (!ok) {
            fprintf(stderr,
                    "ERROR: cannot attach hat%d (no (be,te) places v[%d] at center)\n",
                    k + 1, CENTER_VERTEX);
            return 0;
        }
    }
...
/*
 * Check whether *boundary is invariant under 120° rotation around hat.v[12].
 * The rotation is: apply t=2 lattice rotation, then translate by (g_rot_m6, g_rot_n6).
 */
static int has_120_symmetry(const Poly *boundary, int lattice) {
    ...
    tetrille_translate_cycle(&rotated, g_rot_m6, g_rot_n6);
...
int main(int argc, char **argv) {
    ...
    /* Build the 3-hat pinwheel seed (sharing vertex 12). */
    Cycle center;
    BComp1State seed;
    if (!build_symmetric_seed(&ctx.tile, &seed, &center)) {
        fprintf(stderr, "ERROR: could not build symmetric 3-hat seed at v[%d]\n",
                CENTER_VERTEX);
        bcomp1_context_clear(&ctx); return 1;
    }
    fprintf(stderr,
            "Symmetric 3-hat seed (center=v[%d]=%d(%d,%d), rot_m6=%d rot_n6=%d, tiles=%d)\n",
            CENTER_VERTEX,
            ctx.tile.base.v[CENTER_VERTEX].v,
            ctx.tile.base.v[CENTER_VERTEX].x,
            ctx.tile.base.v[CENTER_VERTEX].y,
            g_rot_m6, g_rot_n6, seed.tile_count);
    ...
    fprintf(stderr,
            "grow_symmetric2  center=v[%d]  level=%d\n"
