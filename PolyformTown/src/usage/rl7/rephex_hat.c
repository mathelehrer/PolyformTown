...
static Point hat_average_center(const Point *hat, int nh) {
    Point p = {0.0, 0.0};
    for (int i = 0; i < nh; i++) { p.x += hat[i].x; p.y += hat[i].y; }
    if (nh > 0) { p.x /= nh; p.y /= nh; }
    return p;
}

static Transform center_hat_transform(const HCell *cell, const Point *hat, int nh) {
