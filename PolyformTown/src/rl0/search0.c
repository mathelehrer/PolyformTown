...
static int rebuild_map0(RL0ForgetMap *map,
    ...
    return rl0_fm_build_from_cycles(map);
}
...
static int write_level0_deletions0(const char *path,
    ...
    fprintf(fp, "# Regenerated by rl0_search0 from strict Level 0 self-check.\n");
...
static int write_search_deletions0(const char *path,
    ...
    if (!fp) return 0;
    fprintf(fp, "# RL0 indexed vertex-figure deletions.\n");
    fprintf(fp, "# Regenerated by rl0_search0.\n");
    fprintf(fp, "# Reflections are generated at dictionary load time.\n\n");
    for (int p = 0; p < SEARCH0_MAX_PROV; p++) {
...
static int regenerate_level0_deletions0(const char *completions_path,
    ...
    if (!raw) return 0;
    rl0_fm_init(raw);
    if (!rl0_fm_load_completions_filtered(raw, completions_path, NULL, -1)) {
        free(raw);
        return 0;
    }
    if (!rl0_fm_build_from_cycles(raw)) {
        free(raw);
        return 0;
    }
    for (int i = 0; i < record_count; i++) {
...
int main(int argc, char **argv) {
    const char *tile_path = argc > 1 ? argv[1] : "tiles/hat.tile";
    const char *completions_path = argc > 2 ? argv[2] : "data/rl0/completions.dat";
    const char *deletions_path = argc > 3 ? argv[3] : "data/rl0/deletions.dat";
    int delete_level = 0;
    int hidden_bound = argc > 4 ? atoi(argv[4]) : 20;
    int focus_record = 0;
    ...
    for (int ai = 5; ai < argc; ai++) {
        if (strcmp(argv[ai], "--print") == 0 && ai + 1 < argc) {
            print_path = argv[++ai];
            continue;
        }
    ...
    if (!parse_records0(completions_path, records, &record_count)) {
        fprintf(stderr, "failed to parse completions\n");
        return 1;
    }
    if (!keep_deletions) {
        if (!regenerate_level0_deletions0(completions_path,
                                          deletions_path,
                                          &tile,
                                          records,
                                          record_count)) {
    ...
    if (!rebuild_map0(map, completions_path, &deletions, delete_level)) {
    ...
    for (int i = 0; i < record_count; i++) {
        ...
        if (optimized && focus_record <= 0 && !optimized_record0(records[i].file_no)) continue;
    ...
    while (cur.count > 0 && step < 1000) {
        ...
        if (global_min == SEARCH0_INF) {
            printf("checkpoint complete: no remaining ports states=%d\n", cur.count);
            break;
        }
        statevec_free0(&next);
        statevec_init0(&next);
