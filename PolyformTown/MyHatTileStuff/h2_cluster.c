 ...
 * h2_cluster.c
 *
 * Build clusters of hat tiles around an h2 metatile center, following the same
 * pipeline as src/rl2/generate_rl2.c but with tiles/h2.tile as the fixed center
 * of the cluster.
 *
 ...
 * be an individual hat.  The h2 metatile has exactly the area of two hats, so
 * the first step here is to DECOMPOSE h2 into its two constituent hat tiles;
 * those two hats form the seed BComp1State whose center is the h2 outline.
 *
 ...
 *
 *   bin/rl1_depict preferences/focus.tile data/h2/completions.dat --grouped \
 *       | bin/imgtable > img/h2_survivors.svg
 *
 * (see the `h2_cluster_svg` Makefile target).
 *
 * Build:  make h2_cluster
 * Run:    ./bin/h2_cluster [--center tiles/h2.tile] [--tile preferences/focus.tile]
...
#define H2_FORCE_STEPS 1024
#define H2_HAT_COUNT   2      /* h2 has exactly two hats' worth of area */
...
/* ---------------------------------------------- h2 -> 3-hat decomposition */

static Cycle     g_h2;                 /* h2 boundary, normalized CCW           */
static long long g_hx[MAX_VERTS];      /* exact embedded x of each h2 vertex    */
static long long g_hy[MAX_VERTS];      /* exact embedded y of each h2 vertex    */
static const Tile *g_hat;              /* hat tile (with its variants)          */

static Cycle     g_pieces[H2_HAT_COUNT]; /* decomposition result                */
...
/* Exact "is q inside or on the h2 boundary?" test on embedded coordinates. */
static int point_inside_or_on(Coord q) {
    long long qx, qy;
    int c = 0;
    tetrille_embed_point_scaled(q, &qx, &qy);
    for (int i = 0; i < g_h2.n; i++) {
        Coord a = g_h2.v[i];
        Coord b = g_h2.v[(i + 1) % g_h2.n];
        if (tetrille_point_on_segment(q, a, b)) return 1;
    }
    for (int i = 0, j = g_h2.n - 1; i < g_h2.n; j = i++) {
...
static int hat_inside_h2(const Cycle *h) {
    for (int i = 0; i < h->n; i++)
 ...
 * that stays inside h2, until the union exactly fills h2 with H2_HAT_COUNT hats. */
static void dfs_decompose(const Poly *agg, int count) {
    if (g_done) return;
    if (count == H2_HAT_COUNT) {
        if (agg->cycle_count == 1 && same_vertex_set(&agg->cycles[0], &g_h2)) {
            g_union = *agg;
            g_done = 1;
        }
        return;
    ...
    for (int be = 0; be < ec && !g_done; be++) {
        for (int vi = 0; vi < g_hat->variant_count && !g_done; vi++) {
            ...
            for (int te = 0; te < var->n && !g_done; te++) {
                ...
                if (!hat_inside_h2(&aligned)) continue;  /* spills outside h2   */
...
/* Decompose h2 into two hats and build the seed BComp1State. */
static int decompose_h2(BComp1State *seed) {
    BComp1Record rec;
    g_done = 0;
    for (int k = 0; k < g_h2.n && !g_done; k++) {
        for (int vi = 0; vi < g_hat->variant_count && !g_done; vi++) {
            const Cycle *var = &g_hat->variants[vi];
            for (int j = 0; j < var->n && !g_done; j++) {
                Cycle cand;
                Poly agg;
                if (!place_variant_on(var, j, g_h2.v[k], &cand)) continue;
                if (!hat_inside_h2(&cand)) continue;
                agg.cycle_count = 1;
                agg.cycles[0] = cand;
                g_pieces[0] = cand;
                dfs_decompose(&agg, 1);
    ...
    rec.have_tiles = 1;
    rec.tiles_count = H2_HAT_COUNT;
    for (int i = 0; i < H2_HAT_COUNT; i++) rec.tiles[i] = g_pieces[i];
    rec.have_center = 1;
    rec.center = g_h2;   /* highlight the whole h2 outline as the cluster center */
...
int main(int argc, char **argv) {
    const char *center_path      = "tiles/h2.tile";
    ...
    /* Normalize the h2 center cycle to CCW and record exact embedded coords. */
    g_h2 = center_tile.base;
    if (cycle_signed_area2(&g_h2, center_tile.lattice) < 0) cycle_reverse(&g_h2);
    cycle_normalize_position(&g_h2, center_tile.lattice);
    for (int i = 0; i < g_h2.n; i++)
        tetrille_embed_point_scaled(g_h2.v[i], &g_hx[i], &g_hy[i]);
    g_hat = &ctx.tile;

    if (!decompose_h2(&seed)) {
        fprintf(stderr, "ERROR: could not decompose %s into %d hats\n",
                center_path, H2_HAT_COUNT);
        bcomp1_context_clear(&ctx);
        return 1;
    }
    center = g_h2;
    fprintf(stderr, "Decomposed %s into %d hats (seed tiles=%d, hidden=%d)\n",
            center_path, H2_HAT_COUNT, seed.tile_count, seed.hidden_count);
